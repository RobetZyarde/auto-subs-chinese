use crate::types::{LabeledProgressFn, ProgressType, Segment, WordTimestamp};
...
pub async fn translate_segments(
    ...
    if total > 0 {
        if let Some(p) = progress {
            p(0, ProgressType::Translate, &start_label);
        }
    }
    ...
    while let Some((k, res)) = stream.next().await {
        ...
        if let Some(p) = progress {
            p(percent.min(99), ProgressType::Translate, start_label);
        }
    }
    ...
    if total > 0 {
        if let Some(p) = progress {
            p(100, ProgressType::Translate, start_label);
        }
    }
